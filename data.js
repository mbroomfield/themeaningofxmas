...
function parseGender(input, isMale, subject) {
    ...
    return input.replace(/\[(.*?)\]/g, (match, inner, inner1) => {
        const gendereds = inner.split("/");
        if (subject) {
            return isMale ? gendereds[0] : gendereds[1];
        } else {
            return isMale ? gendereds[1] : gendereds[0];
        }
    });
}
...
function main() {
    ...
    let conclusion = pickRandom(conclusions);
    maleActor = pickRandom(maleActors);
    femaleActor = pickRandom(femaleActors);

    let intro, descriptor1, descriptor2, person1, person2, adjective;
    if (isMale) { // get random male details
        intro = pickRandom(maleIntros);
        descriptor1 = pickRandom(maleDescriptors1);
        descriptor2 = pickRandom(femaleDescriptors2);
        person1 = `${pickRandom(maleFakeNames)} (${maleActor})`;
        person2 = `${pickRandom(femaleFakeNames)} (${femaleActor})`;
        adjective = pickRandom(femaleAdjectives);
    }
    else { // get random female details
        intro = pickRandom(femaleIntros);
        descriptor1 = pickRandom(femaleDescriptors1);
        descriptor2 = pickRandom(maleDescriptors2);
        person1 = `${pickRandom(femaleFakeNames)} (${femaleActor})`;
        person2 = `${pickRandom(maleFakeNames)} (${maleActor})`;
        adjective = pickRandom(maleAdjectives);
    }

    // resolve gender in conclusion
    conclusion = parseGender(conclusion, isMale, 1);

    const pronoun1 = isMale ? `He` : `She`;
    const firstClause = intro + ` ` + person1 + descriptor1 + `.`;
    const secondClause = pronoun1 + ` meets the ` + adjective + ` ` + person2 + `, ` + descriptor2 + `.`;
    const thirdClause = conclusion;

    const whole = `
        <p>${firstClause}</p>
        <p>${secondClause}</p>
        <p>${thirdClause}, and they fall in love and discover the true meaning of Christmas.</p>`;
